# make_fine_tuning_data_from_python_dict.py
import os
...
def create_finetune_data(data_dict):
    ...
    prompt_q = f"以下のタイトル、サブタイトルからコードが答となる様な質問を1つ作成してください。\n\nタイトル：{data_dict['title']} \n\n サブタイトル：{data_dict['subtitle']} \n\nプログラムコード：{data_dict['code']} "
...
def main():
    # JSONファイルからデータを読み込む。
    with open('./python_code_to_dict.json', 'r') as file:
        data = json.load(file)

    # ファインチューニング用データの生成
    # {"prompt": "prompt-data","completion": "content-data"}
    with open('./program_data.jsonl', 'w', encoding='utf-8') as file:
    ...
    print("ファインチューニング用データが'./program_data.jsonl'に保存されました。")
